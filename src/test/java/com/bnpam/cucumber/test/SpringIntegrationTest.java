...
@SpringBootTest(classes = StudentServicesApplication.class, webEnvironment = WebEnvironment.DEFINED_PORT)
...
public class SpringIntegrationTest {
   ...
   public  void executePost() throws IOException {
        ...
        latestResponse = restTemplate
          .execute("http://localhost:8083/student-service/baeldung", HttpMethod.POST, requestCallback, response -> {
              if (errorHandler.hadError) {
                  return (errorHandler.getResults());
              } else {
                  return (new ResponseResults(response));
